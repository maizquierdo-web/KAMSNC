...
export default function HomePage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const bellOnlyAlertTypes = new Set([
        'channel_reassigned', 'channel_critical_change', 'onboarding_blocked',
        'benchmark_signal', 'team_risk', 'high_potential_movement',
      ]);
      (alertsRes.data || []).forEach(alert => { if (bellOnlyAlertTypes.has(alert.alert_type)) return; if (alert.alert_type === 'channel_inactive' && channelsWithPendingVisit.has(alert.channel_id)) return; if (alert.channel_id && representedChannels.has(alert.channel_id)) return; if (alert.channel_id) representedChannels.add(alert.channel_id); attention.push({ key: `alert:${alert.id}`, alertId: alert.id, severity: alert.priority === 'high' ? 'high' : 'low', channelId: alert.channel_id, channelName: alert.title, title: alert.detail || 'Aviso pendiente', detail: alert.due_date ? `Fecha: ${alert.due_date}` : 'Requiere revisión.' }); });
